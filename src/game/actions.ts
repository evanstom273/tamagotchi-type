...
export function feed(pet: PetState, now = Date.now(), timeScale: SimulationSpeed = 1, worldMinutes?: number): PetActionResult {
  const current = progressPet(pet, now, timeScale, worldMinutes);
...
export function play(pet: PetState, now = Date.now(), timeScale: SimulationSpeed = 1, worldMinutes?: number): PetActionResult {
  const current = progressPet(pet, now, timeScale, worldMinutes);
...
export function clean(pet: PetState, now = Date.now(), timeScale: SimulationSpeed = 1, worldMinutes?: number): PetActionResult {
  const current = progressPet(pet, now, timeScale, worldMinutes);
...
export function toggleSleep(pet: PetState, now = Date.now(), timeScale: SimulationSpeed = 1, worldMinutes?: number): PetActionResult {
  const current = progressPet(pet, now, timeScale, worldMinutes);
